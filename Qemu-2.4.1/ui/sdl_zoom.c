...
int sdl_zoom_blit(SDL_Surface *src_sfc, SDL_Surface *dst_sfc, int smooth,
                  SDL_Rect *in_rect)
{
//printf("sdl_zoom.c - sdl_zoom_blit()\n");
...
//printf("sdl_zoom_blit() - src_rect.x : %d, y : %d, w : %d, h : %d\n", src_rect.x, src_rect.y, src_rect.w, src_rect.h);
    /* (x,y) : round down */
    ...
    zoom.h = (int)( ((double)((src_rect.h * dst_sfc->h) + (src_sfc->h - 1))) /
                     (double)(src_sfc->h));
//printf("sdl_zoom_blit() - zoom.x : %d, y : %d, w : %d, h : %d\n", zoom.x, zoom.y, zoom.w, zoom.h);
     ...
     */
//printf("sdl_zoom.c - sdl~~ -> bpp : %d\n", src_sfc->format->BitsPerPixel);
